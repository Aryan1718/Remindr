...
class FatigueSeed:
    ...
    created_at: datetime
    notes: str | None = None
    source: str = "bulk_seed_script"
    context_json: dict[str, object] | None = None


def build_bulk_scenario(*, now: datetime, row_count: int) -> list[FatigueSeed]:
    templates = [
        {"hour": 8, "score": 1, "label": "morning", "notes": "Morning feels clear"},
        {"hour": 9, "score": 2, "label": "morning", "notes": "Good focus window"},
        {"hour": 14, "score": 4, "label": "afternoon", "notes": "Afternoon slump"},
        {"hour": 15, "score": 5, "label": "afternoon", "notes": "Energy drop after lunch"},
        {"hour": 19, "score": 4, "label": "evening", "notes": "Evening fatigue"},
        {"hour": 21, "score": 5, "label": "night", "notes": "Too tired for deep work"},
    ]
    ...
    for index in range(row_count):
        ...
        created_at = (base_day + timedelta(days=index)).replace(hour=template["hour"])
        if index % 5 == 2:
            created_at = created_at + timedelta(minutes=30)
        seeds.append(
            FatigueSeed(
                score=int(template["score"]),
                created_at=created_at,
                notes=f"{template['notes']} #{index + 1}",
                context_json={
                    "bulk_seed": True,
                    "slot": template["label"],
                    "seed_index": index + 1,
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Seed 50 raw fatigue_checkins rows and invoke fatigue aggregation plus memory distillation."
    )
